(ns shopify.main
  "Kotodama WASM entrypoint for the Shopify clean-room actor (L4) — Clojure port.
  ...
  facts (`shopify.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shopify")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"      :plural "products"      :id-prefix "shopify_pro"
    :fields [:title :vendor :productType :status]  :required [:title :vendor]
    :coerce {}                       :refs {}}
   {:entity "Variant"      :plural "variants"      :id-prefix "shopify_var"
    :fields [:productId :sku :price :inventoryQuantity] :required [:sku :price]
    :coerce {:price :float :inventoryQuantity :int} :refs {:productId "Product"}}
   {:entity "Order"        :plural "orders"        :id-prefix "shopify_ord"
    :fields [:customerId :number :totalPrice :currency :financialStatus] :required [:number :totalPrice]
    :coerce {:totalPrice :float}     :refs {:customerId "Customer"}}
   {:entity "Customer"     :plural "customers"     :id-prefix "shopify_cus"
    :fields [:email :firstName :lastName :ordersCount] :required [:email :firstName]
    :coerce {:ordersCount :int}      :refs {}}
   {:entity "Collection"   :plural "collections"   :id-prefix "shopify_col"
    :fields [:title :handle :published]            :required [:title :handle]
    :coerce {:published :bool}       :refs {}}
   {:entity "Fulfillment"  :plural "fulfillments"  :id-prefix "shopify_ful"
    :fields [:orderId :status :trackingNumber]     :required [:status :trackingNumber]
    :coerce {}                       :refs {:orderId "Order"}}
   {:entity "DiscountCode" :plural "discountcodes" :id-prefix "shopify_dis"
    :fields [:code :valueType :value]              :required [:code :valueType]
    :coerce {:value :float}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"shopify.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shopify-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shopify-compat/ready)
